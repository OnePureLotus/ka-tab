...
vi.mock('@/shared/storage/client', () => ({
  STORAGE_KEYS: {
    ...
    COLLECTION: (id: string) => `local:katab:collection:${id}`,
  },
  storage: {
  ...
  getAllCollections: vi.fn(async () => {
    const index = (storageData.get('local:katab:collections:index') as string[]) ?? []
    return index.map((id) => storageData.get(`local:katab:collection:${id}`)).filter(Boolean)
  }),
  setCollection: vi.fn(async (col: { id: string }) => {
    const index = (storageData.get('local:katab:collections:index') as string[]) ?? []
    if (!index.includes(col.id)) {
      storageData.set('local:katab:collections:index', [...index, col.id])
    }
    storageData.set(`local:katab:collection:${col.id}`, col)
  }),
  setBoard: vi.fn(async (board: { id: string }) => {
    const index = (storageData.get('local:katab:boards:index') as string[]) ?? []
    if (!index.includes(board.id)) {
      storageData.set('local:katab:boards:index', [...index, board.id])
    }
    storageData.set(`local:katab:board:${board.id}`, board)
  }),
}))
...
describe('runBoardMigration', () => {
  it('creates default board when no boards exist', async () => {
    ...
    const boardsIndex = storageData.get('local:katab:boards:index') as string[]
    expect(boardsIndex).toHaveLength(1)
    const board = storageData.get(`local:katab:board:${boardsIndex[0]}`) as { name: string }
  ...
  it('assigns existing collections to default board', async () => {
    ...
    storageData.set('local:katab:collections:index', [col.id])
    storageData.set(`local:katab:collection:${col.id}`, col)

    await runBoardMigration()

    const updated = storageData.get(`local:katab:collection:${col.id}`) as { boardId: string }
    expect(updated.boardId).toBeTruthy()
    const boardsIndex = storageData.get('local:katab:boards:index') as string[]
    const board = storageData.get(`local:katab:board:${boardsIndex[0]}`) as {
  ...
  it('skips when already migrated', async () => {
    ...
    await runBoardMigration()
    expect(storageData.get('local:katab:boards:index')).toBeUndefined()
  })
})
