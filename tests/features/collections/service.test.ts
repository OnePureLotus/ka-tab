...
function makeCollectionWith(n: number): Collection {
  let col = createCollection('Test', '#1a73e8')
  for (let i = 0; i < n; i++) {
    col = addSiteToCollection(col, makeSite({ url: `https://example.com/${i}`, title: `Site ${i}` }))
  }
  return col
}
...
describe('createCollection', () => {
  ...
  it('C-02: 自动映射 tabGroupColor（蓝色 #1a73e8 → blue）', () => {
    const col = createCollection('Test', '#1a73e8')
    expect(col.tabGroupColor).toBe('blue')
  })

  it('C-03: 相同参数生成相同 hash（hash 具确定性）', () => {
    const col1 = createCollection('Work', '#1a73e8')
    const col2 = createCollection('Work', '#1a73e8')
  ...
  it('C-07: 合法 hex color 通过 Schema 校验', () => {
    const col = createCollection('Test', '#4f46e5')
...
describe('renameCollection', () => {
  it('C-21: 重命名后 name 更新', () => {
    const col = createCollection('Old', '#1a73e8')
...
describe('reorderCollections', () => {
  it('C-30: [A,B,C] from=0 to=2 → [B, C, A]', () => {
    const a = createCollection('A', '#1a73e8')
    const b = createCollection('B', '#d93025')
    const c = createCollection('C', '#188038')
...
describe('validateSiteLimit', () => {
  it('C-32: 未达阈值（24 个）→ ok, 无 nearLimit / atLimit', () => {
    ...
    expect(validateSiteLimit(col)).toEqual({ ok: true, count: 24, atLimit: false, nearLimit: false })
  })
