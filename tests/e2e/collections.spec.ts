 ...
 *
 * Context notes:
 *  - EMPTY board   → "No collections in this board" visible, "Create Collection" button
...
test.describe('Collections Board – empty state', () => {
  ...
  test('E2E-C-02: clicking "Create Collection" opens the modal', async ({ newtabPage: page }) => {
    await waitForBoardReady(page)
  ...
  test('E2E-C-03: Create button is disabled when name field is blank', async ({
    newtabPage: page,
  }) => {
    await waitForBoardReady(page)
  ...
  test('E2E-C-04: creating a collection makes its card appear on the board', async ({
    newtabPage: page,
  }) => {
    await waitForBoardReady(page)
  ...
  test('E2E-C-05: pressing Enter in the name field creates the collection', async ({
    newtabPage: page,
  }) => {
    await waitForBoardReady(page)
    await openCreateModal(page)
...
test.describe('Collections Board – with collections', () => {
  ...
  test('E2E-C-06: "Create Collection" card is visible once board has items', async ({
    newtabPage: page,
  }) => {
    await waitForBoardReady(page)
    ...
    await expect(
      page.locator('.collections-panel').getByRole('button', { name: 'Create Collection' }),
    ).toBeVisible()
  })

  // E2E-C-07: clicking "Create Collection" card opens the modal (non-empty context)
  test('E2E-C-07: clicking "Create Collection" card opens the modal', async ({
    newtabPage: page,
  }) => {
    await waitForBoardReady(page)
    await createCollectionViaUI(page, 'Alpha')
  ...
  test('E2E-C-08: counter badge shows correct count after creating two collections', async ({
    newtabPage: page,
  }) => {
    await waitForBoardReady(page)
  ...
  test('E2E-C-09: rename collection via context menu → new name appears on card', async ({
    newtabPage: page,
  }) => {
    await waitForBoardReady(page)
  ...
  test('E2E-C-10: pressing Escape during rename cancels and restores original name', async ({
    newtabPage: page,
  }) => {
    await waitForBoardReady(page)
  ...
  test('E2E-C-11: deleting a collection removes its card from the board', async ({
    newtabPage: page,
  }) => {
    await waitForBoardReady(page)
    ...
    await expect(
      page.locator('[data-collection-id]').filter({ hasText: 'DeleteMe' }),
    ).not.toBeVisible()
  })

  // E2E-C-12: deleting the last collection returns to the empty state
  test('E2E-C-12: deleting the only collection restores the empty state', async ({
    newtabPage: page,
  }) => {
    await waitForBoardReady(page)
  ...
  test('E2E-C-13: "Open all" is disabled when collection has no sites', async ({
    newtabPage: page,
  }) => {
    await waitForBoardReady(page)
  ...
  test('E2E-C-14: clicking "+ Add site" opens the site-add dialog', async ({
    newtabPage: page,
  }) => {
    await waitForBoardReady(page)
  ...
  test('E2E-C-15: adding a valid URL to a collection creates a site entry', async ({
    newtabPage: page,
  }) => {
    await waitForBoardReady(page)
  ...
  test('E2E-C-17: search filters collection cards by name', async ({ newtabPage: page }) => {
    ...
    await expect(
      page.locator('[data-collection-id]').filter({ hasText: 'AlphaProjects' }),
    ).toBeVisible()
    await expect(
      page.locator('[data-collection-id]').filter({ hasText: 'BetaDesign' }),
    ).not.toBeVisible()
  })
